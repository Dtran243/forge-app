...
import type { WeeklySessionPlan, VolumeDecision, LoadDecision, LadderDecision, EngineInputSnapshot } from '../types/engine';
import {
  REP_RANGES,
...
function selectExercise(
  ...
  const filtered = pool.filter((name) => {
    ...
    if (name === 'Hack squat' || name === 'Leg press') {
      // Plate-loaded machines — available when athlete has machines or no barbell rack
    ...
    if (name === 'Bulgarian split squat') {
      // Bodyweight + dumbbells both work; only exclude if dumbbells explicitly false
  ...
  if (filtered.length === 0) {
    console.warn(`selectExercise: no equipment-compatible option found in pool [${pool.join(', ')}], using last entry as fallback.`);
...
/**
 * Parses a timed standard string (e.g. "3x30s") into PlannedSets with hold_duration_seconds.
 * Falls back to a 30s default if parsing fails.
 */
...
function setsForMuscle(
  ...
  // Distribute weekly target across the sessions that train this muscle (2 for Upper/Lower)
...
function loadForMovement(
  ...
  // Try to match by display name against tracked movement keys
  for (const [,] of Object.entries(snapshot.strength)) {
    ...
    // Find matching decision by comparing display names
...
/**
 * Builds the SupersetInfo for embedding a calisthenics ladder movement inside
 * a primary compound exercise card. Only used for superset slots (push/pull/squat ladders).
 */
function buildSupersetInfo(
  ...
  return {
    ...
    hold_duration_seconds: null, // push/pull/squat ladder movements are never timed holds
  };
}
...
/**
 * Builds the standalone core ladder finisher (Slot 6).
 * Core ladder movements are always timed holds — parses the standard string
 * to produce proper hold_duration_seconds sets instead of 1–999 rep ranges.
 */
function buildCalisthenicsFinisher(
  ...
  return {
    name: exerciseName,
    pattern: 'v_pull', // placeholder pattern for skill exercises
    tier: 'accessory',
    pillar: 'skill',
...
function buildUpperA(
  ...
  // Slot 1: Vertical pull (supersetted with pull ladder)
  ...
  // Slot 2: Horizontal push (supersetted with push ladder)
  ...
  // Slot 3: Vertical push (standalone)
  ...
  // Slot 4: Horizontal pull (standalone)
  if (!isDeload) {
    ...
    // Slot 6: Core finisher (standalone timed hold)
...
function buildUpperB(
  ...
  // Slot 1: Horizontal push (supersetted with push ladder)
  const hPush = selectExercise(EXERCISE_POOLS.h_push_primary, snapshot, usedNames);
  usedNames.push(hPush);
  exercises.push({
    name: hPush,
  ...
  // Slot 2: Horizontal pull (supersetted with pull ladder)
  const hPull = selectExercise(EXERCISE_POOLS.h_pull_primary, snapshot, usedNames);
  usedNames.push(hPull);
  exercises.push({
    name: hPull,
...
function buildLowerA(
  ...
  // Slot 1: Hinge primary (supersetted with squat ladder)
  ...
  // Slot 2: Quad primary (standalone — always filled, fallback to Goblet squat if needed)
...
function buildLowerB(
  ...
  // Slot 1: Quad primary (supersetted with squat ladder)
  const quad = selectExercise(EXERCISE_POOLS.quad_primary, snapshot, usedNames);
  usedNames.push(quad);
  exercises.push({
    name: quad,
  ...
  // Slot 2: Hinge primary — prefer RDL in hypertrophy zone
...
function buildZone2Session(): PlannedExercise[] {
  return [{
    ...
    pattern: 'hinge', // placeholder
    tier: 'accessory',
    pillar: 'cardio',
    sets: [{
      set_number: 1,
      target_reps_min: 45,  // minutes
      target_reps_max: 60,
      hold_duration_seconds: null,
      load_kg: null,
      rir_target: 4, // very easy
    }],
    rest_seconds: 0,
    superset: null,
  }];
...
function determineDayCount(
  ...
  if (isDeload) return DAYS_RED; // deload = 3 training days

  const gate = snapshot.recovery.current_gate;

  // Check if multiple pillar gaps exist
  const multipleGaps =
...
function getSessionSequence(dayCount: number, isDeload: boolean, gateIsGreen: boolean): SessionSpec[] {
  if (isDeload) {
    // Deload: UA, LA, Mobility, UB, rest, Mobility, rest
    return [
...
function buildExercisesForLabel(
  ...
  if (label === 'Intervals') return buildZone2Session(); // same structure, different HR target
  return [];
}
